# 🧬 使用MAX30102血氧检测模块与Arduino Uno控制LED
...
- **Arduino Uno开发板** ×1
- **MAX30102心率血氧传感器模块** ×1
- **LED发光二极管**（颜色可选，建议绿色和红色各一个） ×2
- **220Ω 电阻** ×2 (用于限制LED电流，防止损坏)
- **杜邦线** 若干
- **面包板**（可选，方便接线） ×1

## 2.连接线路

MAX30102模块与Arduino Uno主要通过I2C协议通信，LED则需要连接到数字引脚。下面是接线对照表：

| MAX30102引脚 | Arduino Uno引脚 | 其他元件            |
| :----------- | :-------------- | :------------------ |
| VIN (或 VCC) | 5V              |                     |
| GND          | GND             | LED负极（阴极）     |
| SDA          | A4 (SDA)        |                     |
...
> ⚠️ **注意**：
>
> - MAX30102模块**VIN**引脚允许3.3V至5V输入。使用5V供电即可。
> - LED**必须串联一个约220Ω的限流电阻**，直接连接至 Arduino 引脚会损坏板卡或LED。
> - 确保所有GND（Arduino和MAX30102）共地。

下面是MAX30102、Arduino Uno和LED的接线示意图：

![image-20250912150937399](C:\Users\13092\AppData\Roaming\Typora\typora-user-images\image-20250912150937399.png)
...
在Arduino IDE中，你需要安装**MAX30105**库来驱动MAX30102传感器：

1. 打开Arduino IDE。
2. 点击 **项目** -> **加载库** -> **管理库...**。
3. 在搜索框中输入 "**MAX30105**"。
4. 选择 SparkFun 或类似的库进行安装。

## 4.Arduino代码示例

以下代码实现了读取心率，并根据心率值控制两个LED灯（绿色和红色）的功能。
